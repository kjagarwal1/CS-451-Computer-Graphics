 ...
 *
 *
 * Implemented OpenGL style matrix operations:
 * 	1. myLoadIdentity();
 *  2. myTranslatef(); myRotatef(); myScalef();
 *  3. myMultMatrix(); myPushMatrix(); myPopMatrix();
 *  4. myTransHomoVertex(); transform the vertex by the current matrix
 *  5. getMatrix(); get the current matrix on top of the matrix stack;
 *  6. uploadMV(); upload MV matrix to the shaders;
...
public class JOGL2_0_2DTransform extends JOGL1_4_5_Circle {
  ...
  public void display(GLAutoDrawable drawable) {
    ...
    myRotatef((float)cnt/20, 0, 0, 1f); // I*S(x, y, 1)*R(a)
    transDrawTriangle(vdata[0], vdata[1], vdata[2]);
    ...
    myTranslatef((float)cnt/100f, 0f, 0f);// I*S(x, y, 1)*R(a)T(x, 0, 0)
    transDrawTriangle(vdata[0], vdata[1], vdata[2]);
  }
