...
void	main(void)	{	
	...
	tc.x = tc.x + Cnt/2000.0;
	tc.y = tc.y + Cnt/3000.0;
	
 
    vec4 fogColor = texture(fogTex, tc);
 
    // highest density: white, lowest density: black
 	float density = fogColor.x*fogColor.y*fogColor.z/250f; 
 	float vDistance = length(Vg);
 	float f = pow(2.71, -density*vDistance); //e

	//fColor[3] = f;  
    fColor = f*fColor + (1-f)*fogColor; 
    if (normal.z==1.0) fColor = fogColor; // for drawing the background
}